...
echo 'establish release version number via redmine, jruby and java versions (and maybe tomcat version)'
set host = ${HOST:r}
set jrb = '1.8.0'
set jre = '1.8' 
set apollo = '1.7.0'
...
set opt = "show"
if ( "$1" != "" ) set opt = "$1"
set envlist = 'WWW_HOME ASSETS APOLLO_HOME CATALINA GEMFILE HUDSON_HOME HUDSON_OPTS IRCATALINA JAVAREL JAVA_HOME JENKINS_HOME JRUBY JRUBY_HOME JRUBY_OPTS JRUBYREL JXX_OPTS JYTHON_HOME NODE_HOME NODE_PATH NODE_ENV RAILS_ENV RCCATALINA REDMINE REDMINE_LANG REDMINEREL REDMJNE SCM_HOME SCM_OPTS SOLR_HOME SOLR_DATA SOLR_OPTS TOMCAT_HOME TOMCATREL TOMCAT JAVA_OPTS'
...
#
echo 'note the assumption is ~/{scm,solr,www} are sym-links to ~/monthYear/{scm,solr,www}'
...
if ( ! $?IRCATALINA ) setenv IRCATALINA ${WWW_HOME}/${ircatalina} ; echo "IRCATALINA == $IRCATALINA"
if ( ! $?JAVAREL ) setenv JAVAREL $jre ; echo "JAVAREL == $JAVAREL"
#if ( ! $?JAVA_HOME ) setenv JAVA_HOME $cwd/openjdk ; echo "JAVA_HOME == $JAVA_HOME "
if ( ! $?JAVA_HOME ) setenv JAVA_HOME /usr/java/latest ; echo "JAVA_HOME == $JAVA_HOME "
...
if ( ! $?SOLR_HOME ) setenv SOLR_HOME ${HOME}/IR/solr_home ; echo "SOLR_HOME == $SOLR_HOME"
...
# latest 4.x solr only needs home env -- new core.properties establishes dataDir ?
...
#
echo
echo 'default tomcat port 8080 is poduction haproxy backend'
echo 'tomcat release cadidates "rc" use port 8090'
echo 'tomcat irepos -- solr, dspace, vivo -- use port 8070'
